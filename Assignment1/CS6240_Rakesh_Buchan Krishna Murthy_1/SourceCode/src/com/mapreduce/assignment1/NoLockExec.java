...
public class NoLockExec {
	...
	public void initiateProcessing(int noOfThreads) {
		...
		for (int j = 0; j < 10; j++) {
			...
			NoLockWorker processingChunk[] = new NoLockWorker[noOfThreads];
			...
			for (int i = 0; i < noOfThreads; i++) {
				...
				// processors
				processingChunk[i] = new NoLockWorker(startOffset, endOffset);
		...
		System.out.println("No Lock Minimum Execution Time : " + executionStats.get(0));
		System.out.println("No Lock Maximum Execution Time : " + executionStats.get(9));
		System.out.println("No Lock Average Execution Time : " + totalExecTime / 10);
